...
class Feed extends React.Component {
  ...
  render() {
    const { isLoading, newMessages, error, matches } = this.state;

    if (error) {
      return (
        <div>
          Ops something went wrong! Unable to load feed: {error.message}{" "}
        </div>
      );
    }
    ...
    const renderMatches = this.state.matches.map((matchInfo) => {
      return (
        <div key={matchInfo.id} className="match-info">
          <Link
            to={{
              pathname: `/messages/${matchInfo.user_who_matched}`,
              user: {
                image: matchInfo.img_url,
                firstname: matchInfo.firstname,
                surname: matchInfo.surname,
              },
            }}
          >
            <img src={matchInfo.img_url} alt="" />
            <h5>
              {matchInfo.firstname} {matchInfo.surname}
            </h5>
          </Link>
        </div>
      );
    ...
    const renderMessages = messageList.map((listElement) => {
      const lastMessage = listElement[0];
      const myID = this.state.payload.id;

      const displayed_fname =
        lastMessage.from_user_id === myID
          ? lastMessage.to_firstname
          : lastMessage.from_firstname;
      const displayed_lname =
        lastMessage.from_user_id === myID
          ? lastMessage.to_surname
          : lastMessage.from_surname;
      const renderedID =
        lastMessage.from_user_id === myID
          ? lastMessage.to_user_id
          : lastMessage.from_user_id;
      const matchedUserImg =
        lastMessage.from_user_id === myID
          ? lastMessage.to_img_url
          : lastMessage.from_img_url;
      ...
      return (
        <div key={lastMessage.id}>
          <Link
            to={{
              pathname: `/messages/${renderedID}`,
              user: {
                image: matchedUserImg,
                firstname: displayed_fname,
                surname: displayed_lname,
              },
            }}
          >
            <div className="message-cards">
              ...
              <div className="message-text">
                <h5>
                  {displayed_fname} {displayed_lname}
                </h5>
                {lastMessage.message} - {timeAgo}
              </div>
            </div>
          </Link>
        </div>
